...
/*
 * @dev "mocha -r ts-node/register test/_mainnet-only/sushi-swap.test.ts --timeout 2000000 --exit --recursive"
 */
describe("Adapter - SushiSwap", () => {
  let accounts: any;
  let owner: any;
  ...
  before("deploy dao", async () => {
    ...
    unirouter = await getContractByName("IUniswapV2Router").at("0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F");
  ...
  it("should be possible to create a SushiSwap ETH/DAI Add Pool proposal, process it and move the ETH/DAI funds to the pool", async () => {
    await buyToken(DAI);
    ...
    const adapter = adapters.sushiswapAddPoolAdapter;
  ...
  it("should be possible to create a SushiSwap ETH/DAI Sub Pool proposal, process it and move the ETH/DAI funds from the pool", async () => {
    const proposalId = generator().next().value;
    const adapter = adapters.sushiswapSubPoolAdapter;
  ...
  it("should be possible to create a SushiSwap USDC/DAI Add Pool proposal, process it and move the USDC/DAI funds to the pool", async () => {
    await buyToken(USDC);
    await buyToken(DAI);
  ...
  it("should be possible to create a SushiSwap USDC/DAI Sub Pool proposal, process it and move the USDC/DAI funds from the pool", async () => {
    const proposalId = generator().next().value;
    const adapter = adapters.sushiswapSubPoolAdapter;
