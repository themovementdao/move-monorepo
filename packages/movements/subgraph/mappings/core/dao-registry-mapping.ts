...
export function handleSubmittedProposal (event: SubmittedProposal): void {
  const submittedBy = event.transaction.from;
  const proposalId = event.params.proposalId;
  const daoAddress = event.address;
  const daoProposalId = daoAddress
    .toHex()
    .concat('-proposal-')
  ...
  const daoRegistry = DaoRegistry.bind(event.address);
  const data = daoRegistry.proposals(event.params.proposalId);
  ...
  const adapterAdddress = data.value0;

  const inverseAdapter = daoRegistry.inverseAdapters(adapterAdddress);
...
export function handleSponsoredProposal (event: SponsoredProposal): void {
  ...
  const daoAddress = event.address.toHex(); // dao contract address
  const newProposalId = daoAddress.concat('-proposal-').concat(id.toHex());

  const proposal = Proposal.load(newProposalId);
  const sponsoredAt = event.block.timestamp.toString();
...
export function handleProcessedProposal (event: ProcessedProposal): void {
  const processedAt = event.block.timestamp.toString();

  log.info('=============== ProcessedProposal event fired. proposalId: {}', [
    event.params.proposalId.toHexString()
  ]);

  const proposal = loadProposalAndSaveVoteResults(
    event.address,
...
export function handleUpdateDelegateKey (event: UpdateDelegateKey): void {
  ...
  const delegateKey = event.params.newDelegateKey;
  const memberId = event.params.memberAddress.toHex();
...
export function handleAdapterAdded (event: AdapterAdded): void {
  const daoAddress = event.address.toHexString();
  const adapterId = event.params.adapterId;
  const daoAdapterId = daoAddress.concat('-adapter-').concat(adapterId.toHex());
...
export function handleAdapterRemoved (event: AdapterRemoved): void {
  ...
  const adapter = Adapter.load(daoAdapterId);
...
export function handleExtensionAdded (event: ExtensionAdded): void {
  const daoAddress = event.address.toHexString();
  const extensionId = event.params.extensionId;

  const daoExtensionId = daoAddress
    .concat('-extension-')
...
export function handleExtensionRemoved (event: ExtensionRemoved): void {
  ...
  const extension = Extension.load(daoExtensionId);
