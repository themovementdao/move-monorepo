...
export function getAdapterAddressFromContracts(
  ...
  try {
    switch (name) {
      ...
      case ContractAdapterNames.movement:
        return getContractAddressOrThrow(contracts.MovementContract);
      case ContractAdapterNames.unagii_deposit:
        return getContractAddressOrThrow(contracts.UnagiiDeposit);
      case ContractAdapterNames.unagii_withdraw:
        return getContractAddressOrThrow(contracts.UnagiiWithdraw);   
      default:
        throw new Error(`Contract address was not found in the store.`);
    }
  } catch (error) {
