...
import {
  ...
  initContractUnagiiVaultDeposit,
  initContractUnagiiVaultWithdraw
} from '../../../store/actions';
...
export function useInitContracts(): (data: {
  ...
  async function initContracts({web3Instance}: {web3Instance: Web3}) {
    try {
      ...
      await dispatch(initContractMovement(web3Instance));
      await dispatch(initContractUnagiiVaultDeposit(web3Instance));
      await dispatch(initContractUnagiiVaultWithdraw(web3Instance));
    } catch (error) {
      throw error;
    }
  }
