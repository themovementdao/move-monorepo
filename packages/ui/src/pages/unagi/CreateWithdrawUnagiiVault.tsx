...
export default function CreateWithdrawUnagiiVault(): JSX.Element {
     /**
        * Selectors
     */
    const history = useHistory();
    const UnagiiWithdrawAdapter = useSelector(
        (state: StoreState) => state.contracts?.UnagiiWithdraw
    );
    ...
    const { UNAGII_DAI_VAULT_ADDRESS, uTokenBalance, account, web3Instance } = useUnagiiVaults();
    ...
    const {errors, getValues, setValue, register } = form;

    const [isLoading, setIsLoading] = useState(false);

    useEffect(() => {
        const checkBalance = async () => {
            if (UnagiiWithdrawAdapter?.contractAddress) {
                setBalance(await uTokenBalance(UnagiiWithdrawAdapter?.contractAddress));
            }
        }
        checkBalance();
    }, [UnagiiWithdrawAdapter]);

    async function handleSubmit(values: FormInputs) {
        try {
            if (!UnagiiWithdrawAdapter) {
            ...
            if (!proposalId) {
                ...
                const {
                  ...
                  signature: returnSignature,
                    ...
                    partialProposalData: {
                      name: 'Withdraw unagii vaults',
                      body: `Withdraw unagii vaults amount ${amount}`,
                      metadata: {
                        amount
                      },
                      timestamp: now.toString(),
                    },
                    adapterName: ContractAdapterNames.unagii_withdraw,
            ...
            */
            ...
            await txSend(
                'submitProposal',
                UnagiiWithdrawAdapter.instance.methods,
                transferArguments,
                txArguments
            );
      ...
      <Wrap className="section-wrapper">
        <FadeIn>
          <div className="titlebar">
            <h2 className="titlebar__title">Unagii Vault Withdraw Proposal</h2>
          </div>
